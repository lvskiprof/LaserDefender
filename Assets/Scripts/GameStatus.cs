...
public class GameStatus : MonoBehaviour
{
	...
	***/
	...
	[SerializeField]
	long	currentScore = 0;
	bool	allEnemiesDestroyed = false;    // This can be used to tell if all the blocks on the last level have been destroyed
	...
	*		Note: DestroyImmediate() is used instead of Destroy(), because there can be a
	*	delay with Destroy() that can cause problems if some code accesses the wrong the
	*	newer instance before it is destroyed.  However, I was told that DestroyImmediate
	*	() can have bad side effects, so I am not using it.  Another option can be to set
	*	the gameObject to be inactive before it is destroyed.  That code would look like
	*	this:
    ...
    *       It also detects if we are on the last scene by checking for a Text object with
    *   a tag of "Finish".    This is the text that defaults to "You Lost!!!".  If the
    *   flag is set that you destroyed all the balls on the last level it will change the
    *   text to "You Won!!!".
    ***/
	void Update()
	{
	...
	/***
	*       AddToScore() adds to the currentScore the points for the block that was
	*	destroyed.  
	*       Note:  Later we may want to pass a value based on the type of block that was
	*	destroyed.  Right now we are using the blockColor value (a number from 0 to N),
	*	adding 1 to it, and using it to multiply the point value.  If we want, this could
	*	be changed to be a enemyValue, but tying it to the color will make visual sense to
	*	the player.
	***/
	...
	***/
	...
	/***
	*       SetAllEnemiesDestroyed() sets  allEnemiesDestroyed to the passed state.
	***/
	...
	/***
	*       AllEnemiesDestroyed() returns the state of allEnemiesDestroyed to the caller.
	***/
